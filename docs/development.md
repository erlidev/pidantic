...
npm run smoke:sandbox                             # run real commands in a real bubblewrap namespace
```
...
The explicit test glob excludes every `smoke.ts` file. Localsearch smoke commands perform live
...
configuration, but does not send a model request. The sandbox smoke command needs Linux with
`bubblewrap` installed and unprivileged user namespaces enabled; it is the only check that tests what
the kernel does with the command line the package builds rather than the shape of that command line,
so run it after touching anything under `safety/src/sandbox/`. The sandbox cases in
`safety/test/sandbox-gate.test.ts` skip rather than fail where bubblewrap is unusable, so a clean
`npm test` on a machine without it does not mean confinement was verified.
