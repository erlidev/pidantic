 ...
 * owns it and which project it belongs to: `<temp>/pi-scratchpad-<uid>/<project>-<hash>/<session>`.
 * The uid is in the top level because `/tmp` is shared — a directory another user created first
 * would otherwise be un-writable rather than merely someone else's. The project level is named
 * after the workspace so a stray directory can be identified by eye, and disambiguated by a hash of
 * the absolute path, since two checkouts of one repository share a basename.
 */
...
export interface ScratchpadPathOptions {
...
export function scratchpadPath(options: ScratchpadPathOptions): string {
	...
	const base = options.baseDir?.trim() ? options.baseDir.trim() : join(options.tmp ?? tmpdir(), `pi-scratchpad-${uid}`);
